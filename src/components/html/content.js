        ...
        for (let i in this.content) {
            ...
            switch (c.type) {
                ...
                case PRISMIC_MARKUP_HEADING1:
                    render.push(<h1 key={'content-heading-' + i} className={'text-4xl content-heading-' + i}>{c.text}</h1>)
                    break;
                case PRISMIC_MARKUP_HEADING2:
                    render.push(<h2 key={'content-heading-' + i} className={'text-3xl content-heading-' + i}>{c.text}</h2>)
                    break;
                case PRISMIC_MARKUP_HEADING3:
                    render.push(<h3 key={'content-heading-' + i} className={'text-2xl content-heading-' + i}>{c.text}</h3>)
                    break;
                case PRISMIC_MARKUP_HEADING4:
                    render.push(<h4 key={'content-heading-' + i} className={'font-bold content-heading-' + i}>{c.text}</h4>)
                    break;
                case PRISMIC_MARKUP_IMAGE:
                    render.push(<img key={'content-image-' + i} className={'content-image-' + i} src={c.url} alt={c.alt} width={c.dimensions.width * 0.90} height="auto" />)
                    break;
                default:
